...
> ⚠️ **حافظه = یادگیری آزاد.** *(تأیید صاحب پروژه)*
...
## دامنه‌ی اجرا (تصمیم صاحب پروژه)

این شش Engine به‌طور کامل فقط برای یک نوع Agent جدا به اسم **Persona Agent** (کاربر مصنوعی) است:
Agentی که نقش مشتری را بازی می‌کند و محصول را تست می‌کند.

Agentهای متخصص (Research، Product، Builder، Reviewer و …) از این اصول فقط این‌ها را می‌گیرند:
- نتیجه‌گیری معلق — بدون شاهد کافی ادعای قطعی نکن (Synthesis)
- ثبت تجربه در حافظه (Memory / Log)
